...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error(
    "Missing Supabase keys in environment after loading .env.local",
  );
...
async function seedFinanceData() {
  ...
  const checkingDescriptions = [
    ...
    {
      desc: "Subcontractor - Mike",
      type: "expense",
      cat: "Subcontractors",
      range: [500, 2000],
    },
    {
      desc: "Subcontractor - Dave",
      type: "expense",
      cat: "Subcontractors",
      range: [400, 1500],
    },
    {
      desc: "Deposit - Smith Job",
      type: "income",
      cat: "Sales / Revenue",
      range: [1000, 5000],
    },
    {
      desc: "Payment - Jones Reno",
      type: "income",
      cat: "Sales / Revenue",
      range: [2000, 8000],
    },
    {
      desc: "E-Transfer - Small Fix",
      type: "income",
      cat: "Sales / Revenue",
      range: [100, 500],
    },
  ...
  const generateTransactions = (
    ...
    for (let i = 0; i < count; i++) {
      ...
      const catId =
        Math.random() > 0.3 && categoryMap.has(tmpl.cat)
          ? categoryMap.get(tmpl.cat)
          : null;
  ...
  const checkingTxns = generateTransactions(
    100,
    "checking",
