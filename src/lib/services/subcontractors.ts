...
import { SubcontractorSchema, type Subcontractor } from '../schemas/subcontractorSchema';

export async function getSubcontractors(organizationId: string): Promise<Subcontractor[]> {
...
export async function updateSubcontractor(id: string, input: Partial<Subcontractor>): Promise<Subcontractor> {
    ...
    if (!input.organization_id) {
        throw new Error("organization_id is required to update subcontractor to ensure tenant boundaries.");
    }
