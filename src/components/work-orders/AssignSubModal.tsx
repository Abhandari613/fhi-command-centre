...
export function AssignSubModal({ isOpen, onClose, jobId, onAssigned }: AssignSubModalProps) {
            ...
            <DialogContent className="bg-black/90 border-white/10 text-white sm:max-w-md">
                ...
                <div className="space-y-4 max-h-[60vh] overflow-y-auto">
                    {loading ? (
                        ...
                        <div className="text-center opacity-50 p-4">No subcontractors found. Add one in the Team tab.</div>
                    ) : (
                        subs.map(sub => (
                            <div
                                key={sub.id}
                                ...
                                <GlassCard className="p-3 flex justify-between items-center hover:bg-white/10 transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs opacity-50">{sub.phone || sub.email}</div>
                                        </div>
                                    </div>
                                    {assigning === sub.id ? (
