...
import { Camera, Trash2, Plus, X, Loader2, Image as ImageIcon } from "lucide-react";
...
export function JobPhotoGallery({ jobId, photos, type, title, readOnly = false }: JobPhotoGalleryProps) {
    ...
    const displayPhotos = photos.filter(p => p.type === type || (!p.type && type === 'other') || p.type === 'before' || p.type === 'after'); // Explicitly allow all known types to bypass overlap check if types are loose
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            reader.onload = async () => {
                ...
                if (!result.success) throw new Error(typeof result.error === 'object' ? (result.error as any)?.message : String(result.error || "Upload failed"));
    ...
    const handleDelete = async (photoId: string) => {
        ...
        try {
            ...
            if (!result.success) throw new Error((result as any).error ? String((result as any).error?.message || (result as any).error) : "Delete failed");
