...
export function CompoundingCurve({ data, className }: CompoundingCurveProps) {
    if (!data || data.length === 0) {
        return (
            <GlassCard className={`p-6 flex items-center justify-center min-h-[300px] ${className}`}>
    ...
    const maxValue = Math.max(...data.map(d => Math.max(d.baseline, d.actual, d.projected || 0))) * 1.1;
    const height = 300;
    ...
    // Generate paths
    const baselinePath = data.map((d, i) => `${i === 0 ? 'M' : 'L'} ${xScale(i)} ${yScale(d.baseline)}`).join(" ");
    ...
    return (
        <GlassCard className={`p-6 ${className}`}>
            ...
            <p className="text-sm opacity-60 mb-6">Visualizing the gap between the old way and the new operations.</p>
