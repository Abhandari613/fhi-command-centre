...
export function ProposalGenerator({
  ...
  const totalValue = interventions.reduce(
    (sum, i) => sum + (i.projected_monthly_value || 0),
    0,
  );
  const totalHours = interventions.reduce(
    (sum, i) => sum + (i.projected_weekly_hours_saved || 0),
    0,
  );
      ...
      <GlassCard className="p-12 space-y-12 bg-black/40 border-white/5 print:border-none print:shadow-none print:text-black">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-xl text-white/60">
            Prepared for {engagement.client_name}
          </p>
        </div>
        ...
        <section className="grid md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <p className="text-white/70 leading-relaxed">
              Based on our discovery session, we have identified key operational
              frictions limiting your growth. This proposal outlines a strategic
              intervention plan designed to reclaim time and unlock revenue
              potential.
            </p>
          </div>
        ...
        <section className="space-y-8">
          ...
          {(Object.entries(interventionsByType) as [string, any[]][]).map(
            ([type, items]) => (
              <div key={type} className="space-y-4">
                ...
                <div className="grid gap-4 pl-4 border-l border-white/10 ml-1">
                  {items.map((item) => (
                    <div
                      key={item.id}
                      className="bg-white/5 rounded-lg p-5 border border-white/5 hover:bg-white/10 transition-colors"
                    >
                      <div className="flex justify-between items-start mb-2">
                        <h5 className="font-bold text-white">{item.name}</h5>
                        <span className="text-xs bg-black/20 px-2 py-1 rounded text-white/40">
                          Phase {item.phase || 1}
                        </span>
                      </div>
        ...
        <div className="pt-8 border-t border-white/10 text-center">
          <p className="text-white/40 text-sm">
            Generated by Frank Home Improvement Outcome Engine
          </p>
        </div>
      </GlassCard>
    </div>
