    ...
    <section className="space-y-4">
      ...
      <div className="grid gap-3">
        ...
        {existingFriction.map((item) => (
          <GlassCard
            key={item.id}
            className="p-4 flex items-start gap-3 animate-in fade-in slide-in-from-bottom-2"
          >
            <AlertTriangle
              className={`w-5 h-5 mt-0.5 ${getSeverityColor(item.risk_impact > 7 ? "critical" : "medium").split(" ")[0]}`}
            />
            <div className="flex-1">
              <p className="text-white/90">{item.description}</p>
              <div className="flex items-center gap-2 mt-2">
                <span
                  className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded ${getSeverityColor(item.severity || "medium")}`}
                >
