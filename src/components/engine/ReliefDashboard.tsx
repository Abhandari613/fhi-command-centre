...
export function ReliefDashboard() {
  ...
  const primaryMetric =
    data.metrics.find(
      (m: any) => m.metric_type === "time" || m.metric_type === "financial",
    ) || data.metrics[0];
    ...
    <div className="space-y-6">
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="opacity-70">
            Tracking value delivery for {data.engagement.client_name}
          </p>
        </div>
        <div className="text-right">
          <div className="text-sm opacity-50 uppercase tracking-wider">
            Phase
          </div>
          <div className="font-bold text-emerald-400">
            {data.engagement.phase}
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <GlassCard className="p-6 flex flex-col justify-between bg-primary/5 border-primary/20">
          <div>
            <span className="text-xs uppercase tracking-wider opacity-60 text-primary">
              Next Calibration
            </span>
            <div className="flex items-end gap-2 mt-2">
          ...
          <button
            className="mt-4 text-xs bg-primary text-black font-bold py-2 px-4 rounded hover:opacity-90 transition-opacity w-full"
            onClick={() => (window.location.href = "/engine/calibrate")}
          >
            Start Session now
          </button>
        </GlassCard>
          ...
          <section>
            ...
            <div className="space-y-3">
              {data.frictions.length === 0 ? (
                <GlassCard className="p-4 text-center opacity-60 text-sm">
                  No active friction.
                </GlassCard>
              ) : (
                data.frictions.map((f: any) => (
                  <GlassCard
                    key={f.id}
                    className="p-4 border-l-4 border-l-amber-500 flex justify-between items-start gap-3"
                  >
                    <div>
            ...
            return (
              <GlassCard key={m.id} className="p-4">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs bg-white/10 px-2 py-1 rounded uppercase">
                    {m.unit}
                  </span>
                </div>
