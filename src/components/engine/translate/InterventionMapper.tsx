...
export function InterventionMapper({ engagementId, unmappedFriction, existingInterventions }: InterventionMapperProps) {
    ...
    const [interventionType, setInterventionType] = useState<'automation' | 'process_change' | 'tool_integration' | 'training' | 'strategic_shift'>('process_change');
    const [isSubmitting, setIsSubmitting] = useState(false);
    const router = useRouter();

    const selectedFrictionItem = unmappedFriction.find(f => f.id === selectedFrictionId);
                ...
                <div className="grid gap-3">
                    {unmappedFriction.map((item) => (
                        <div
                            ...
                            className={`p-4 rounded-xl cursor-pointer transition-all border ${selectedFrictionId === item.id
            ...
            <div className="space-y-4">
                ...
                <GlassCard className="p-6 h-full flex flex-col">
                    {!selectedFrictionId ? (
                        ...
                        <form onSubmit={handleCreateIntervention} className="flex-1 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
                            <div>
                                <label className="text-xs uppercase tracking-wider text-white/40 mb-1 block">Solving For</label>
                            ...
                            <div className="space-y-3 mt-4">
                                <div>
                                    <label className="text-sm text-white/70 block mb-1">Intervention Name</label>
                                    <input
                                        type="text"
