...
} from "@/app/actions/engine/metric-actions";
import {
  getFrictionItems,
  updateFrictionStatus,
} from "@/app/actions/engine/friction-actions";
...
export function CalibrationWizard({ engagementId }: { engagementId: string }) {
  ...
  const handlePlanComplete = async () => {
    ...
    try {
      ...
      const cycle = await createCalibrationCycle(
        engagementId,
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <AnimatePresence mode="wait">
        {step === "intro" && <IntroStep onStart={handleStart} key="intro" />}
        {step === "metrics" && (
          <MetricsStep
            onNext={handleMetricsComplete}
        ...
        {step === "frictions" && (
          <FrictionStep
            onNext={handleFrictionsComplete}
            engagementId={engagementId}
            key="frictions"
          />
        )}
        ...
        {step === "summary" && <SummaryStep cycleId={cycleId} key="summary" />}
      </AnimatePresence>
    </div>
  );
}
    ...
    >
      <GlassCard className="p-12 text-center space-y-6">
        ...
        <p className="text-lg opacity-70 max-w-lg mx-auto">
          Time to review your progress. We'll check your relief metrics, review
          resolved frictions, and plan the next set of interventions.
        </p>
    ...
    >
      <GlassCard className="p-8">
        ...
        <p className="opacity-60 mb-8">
          Update your key relief metrics for this week.
        </p>

        <div className="space-y-4 mb-8">
          {metrics.length === 0 ? (
            <div className="p-4 border border-white/10 rounded-lg text-center opacity-50">
  ...
  return (
    ...
    >
      <GlassCard className="p-8">
        ...
        <p className="opacity-60 mb-8">
          Mark any frictions you've resolved this past week.
        </p>
        ...
        <div className="space-y-3 mb-8 max-h-[300px] overflow-y-auto custom-scrollbar">
          {frictions.length === 0 ? (
            ...
            frictions.map((item) => (
              ...
              >
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center border ${item.status === "resolved" ? "bg-emerald-500 border-emerald-500" : "border-white/20"}`}
                >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs opacity-40 mt-1 uppercase">
                    {item.severity}
                  </div>
                </div>
              </div>
    ...
    >
      <GlassCard className="p-8">
        <h3 className="text-xl font-bold mb-4">
          Step 3: Plan Next Interventions
        </h3>
        <p className="opacity-60 mb-8">
          Select the top 3 interventions to focus on this week.
        </p>

        <div className="p-12 border border-dashed border-white/10 rounded-xl text-center mb-8">
          <span className="opacity-50">
            Intervention selection will go here...
          </span>
        </div>
  ...
  return (
    ...
    >
      <GlassCard className="p-12 text-center">
        ...
        <p className="opacity-60 mb-8">
          Your progress has been recorded. See you next week.
        </p>
        <div className="text-xs opacity-30 font-mono mb-8">
          Cycle ID: {cycleId}
        </div>

        <AnimatedButton
          variant="secondary"
          onClick={() => (window.location.href = "/engine/dashboard")}
        >
          Go to Dashboard
        </AnimatedButton>
      </GlassCard>
