...
import { User, Phone, Mail, Plus, Trash2, Loader2, Search, MapPin, MessageSquare, Hammer } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { getSubcontractors, createSubcontractor, deleteSubcontractor, Subcontractor, CreateSubInput } from "@/app/actions/sub-actions";
...
export function SubList() {
    ...
    if (loading) return <div className="flex justify-center p-8"><Loader2 className="animate-spin w-8 h-8 text-primary" /></div>;
        ...
        <div className="space-y-6">
                ...
                {isAdding && (
                    ...
                    >
                        <GlassCard intensity="panel" className="p-6 mb-6 border-primary/30">
                            <h3 className="font-bold text-lg mb-4 text-white">New Team Member</h3>
                            <form onSubmit={handleAddSub} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-bold uppercase opacity-70 mb-1 tracking-wider">Name</label>
                                        <input
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold uppercase opacity-70 mb-1 tracking-wider">Trade / Role</label>
                                        <input
                                            className="w-full bg-black/20 border border-white/10 rounded-lg p-3 focus:outline-none focus:border-primary/50 text-sm"
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold uppercase opacity-70 mb-1 tracking-wider">Address</label>
                                    <GoogleAddressInput
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-bold uppercase opacity-70 mb-1 tracking-wider">Email</label>
                                        <input
                                            className="w-full bg-black/20 border border-white/10 rounded-lg p-3 focus:outline-none focus:border-primary/50 text-sm"
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold uppercase opacity-70 mb-1 tracking-wider">Phone</label>
                                        <input
                                            className="w-full bg-black/20 border border-white/10 rounded-lg p-3 focus:outline-none focus:border-primary/50 text-sm"
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold uppercase opacity-70 mb-1 tracking-wider">Communication</label>
                                        <GlassSelect
                                            value={newSub.communication_preference}
                                            onChange={(val) => setNewSub({ ...newSub, communication_preference: val as any })}
                                            options={[
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="grid gap-3">
                {subs.length === 0 && !loading ? (
                    <motion.div variants={item} className="text-center opacity-50 py-10">No subcontractors found. Add one to get started.</motion.div>
                ) : (
                    subs.map(sub => (
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-base text-white flex items-center gap-2">
                                            {sub.name}
                                            {sub.trade && <span className="text-[10px] bg-white/10 px-2 py-0.5 rounded-full text-white/70 uppercase tracking-wide">{sub.trade}</span>}
                                        </h3>
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs opacity-60 mt-1">
                                            ...
                                            {sub.address && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" /> {sub.address}</span>}
                                            <span className="flex items-center gap-1"><MessageSquare className="w-3 h-3" /> Pref: {sub.communication_preference}</span>
                                            {sub.compliance_status && (
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${sub.compliance_status === 'verified' ? 'bg-emerald-500/10 text-emerald-400' :
                                                        sub.compliance_status === 'expired' ? 'bg-red-500/10 text-red-400' :
