...
import { Check, X, ArrowRight, AlertTriangle, Building, CreditCard, Wand2, ArrowLeftRight } from 'lucide-react';
import { matchTransactionToWorkOrder, updateTransactionCategory, createCategorizationRule } from '@/app/actions/finance';
...
export function TransactionList({
    ...
    const [rulePrompt, setRulePrompt] = useState<{ txnId: string, description: string, categoryId: string, categoryName: string } | null>(null);
    ...
    const confirmRuleCreation = async () => {
        ...
        try {
            await createCategorizationRule(rulePrompt.description, rulePrompt.categoryId);
    ...
    const handleMatchWorkOrder = async (txnId: string, workOrderId: string) => {
        ...
        if (txnIndex > -1) {
            ...
            setTransactions(nextTxns);
            if (selectedIndex >= nextTxns.length) setSelectedIndex(Math.max(0, nextTxns.length - 1));
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-zinc-900 border border-white/10 p-6 rounded-2xl shadow-2xl max-w-md w-full space-y-4 animate-in zoom-in-95 duration-200">
                        <div className="flex items-center gap-3 text-emerald-400 mb-2">
                            ...
                            <h3 className="text-lg font-bold text-white">Teach the System?</h3>
                        </div>

                        <p className="text-zinc-400">
                            You just categorized <strong className="text-white">"{rulePrompt.description}"</strong> as <strong className="text-emerald-400">{rulePrompt.categoryName}</strong>.
                        </p>
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div className={cn(
                                ...
                                {isExpense ? <CreditCard className="w-5 h-5" /> : <Building className="w-5 h-5" />}
                            </div>
                        ...
                        {txn.suggested_work_order && (
                            <div className="mt-2 pl-[56px] flex items-center gap-3 animate-in slide-in-from-top-1 fade-in">
                                ...
                                <span className="text-sm text-indigo-200">
                                    Match to Work Order: <span className="font-bold text-white">{txn.suggested_work_order.property_address_or_unit}</span>
                                </span>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
