...
export function QuoteBuilder({ initialClientId }: QuoteBuilderProps) {
  ...
  const handleVoiceRecording = async (audioBlob: Blob) => {
    ...
    try {
      ...
      alert(
        `Voice Command Failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
  ...
  return (
    <div className="min-h-screen py-8 px-4 flex items-center justify-center">
      ...
      <GlassCard
        className="max-w-2xl w-full mx-auto p-4 md:p-8 space-y-8"
        intensity="solid"
      >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-xl p-4 flex flex-col items-center justify-center text-center space-y-3">
                  ...
                  <p className="text-xs text-gray-500">
                    Describe the job to auto-fill details
                  </p>
                </div>
            ...
            >
                    ...
                    <GlassCard key={item.id} className="p-4" intensity="bright">
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  updateLineItem(
                                    item.id,
                                    "provided_by",
                                    e.target.value as "contractor" | "client",
                                  )
                                }
                                className="bg-black/20 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white uppercase tracking-wider font-semibold focus:outline-none focus:ring-1 focus:ring-primary/50"
                              >
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div className="col-span-1">
                          <label className="text-[10px] opacity-50 uppercase font-bold block mb-1">
                            Qty
                          </label>
                          <input
                            type="number"
              ...
              <section className="pt-6 border-t border-white/10 mt-8">
                ...
                <div className="mb-8 p-4 bg-white/5 rounded-xl border border-white/10 space-y-4">
                  <div className="flex items-center justify-between">
                    <div
                      className="flex items-center gap-4 border border-white/20 p-4 rounded-xl cursor-pointer"
                      onClick={() => setDepositRequired(!depositRequired)}
                    >
                      <div
                        className={cn(
                          "w-5 h-5 rounded border flex flex-shrink-0 items-center justify-center transition-colors",
                          depositRequired
