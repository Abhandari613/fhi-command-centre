...
export function ClientPicker({ onSelect, selectedClientId }: ClientPickerProps) {
        ...
        return (
            <GlassCard className="p-4 animate-in fade-in zoom-in">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setIsCreating(false)} className="p-1 hover:bg-white/10 rounded-full">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                <div className="space-y-4">
                    <div>
                        <label className="text-xs opacity-60 uppercase font-bold tracking-wider">Client Name</label>
                        <input
                ...
                {filteredClients.map(client => (
                    <button
                        ...
                        className={cn(
                            ...
                            selectedClientId === client.id ? "bg-white/10 border border-white/5" : "hover:bg-white/5"
                        )}
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] opacity-50 mt-1">{client.email || "No email"}</p>
                            </div>
                        </div>
                        {selectedClientId === client.id && <Check className="w-4 h-4 text-emerald-400" />}
                    </button>
                ))}
            </div>
        </div>
