...
// ------------------------------------------------------------------
export const PropertyIntelCard = ({
  formData,
  updateNested,
  isEditing,
  mode = "admin",
  ...
  return (
    <GlassCard className="p-6 space-y-4">
      <h2 className="text-lg font-bold flex items-center gap-2 text-blue-300 border-b border-white/10 pb-2">
        ...
        {mode === "client" ? "Your Home" : "Property Intel"}
      </h2>
      <div className="grid grid-cols-2 gap-4">
          ...
          {isEditing ? (
            ...
            <div className="bg-blue-500/10 text-blue-300 font-mono text-lg px-3 py-2 rounded border border-blue-500/20 flex justify-between items-center group">
              ...
              {formData.property_details?.access_instructions && (
                <Copy
                  onClick={() =>
                    handleCopy(formData.property_details.access_instructions)
                  }
                  className="w-4 h-4 cursor-pointer opacity-50 group-hover:opacity-100"
                />
              )}
            </div>
          )}
        ...
        <EditField
          label="Wifi Network"
          icon={Wifi}
          value={formData.property_details.wifi_network}
          onChange={(v: string) =>
            updateNested("property_details", "wifi_network", v)
          }
          isEditing={isEditing}
        />
        <EditField
          label="Wifi Password"
          icon={Key}
          value={formData.property_details.wifi_password}
          onChange={(v: string) =>
            updateNested("property_details", "wifi_password", v)
          }
          isEditing={isEditing}
        />
        ...
        <EditField
          ...
          onChange={(v: string) => updateNested("property_details", "age", v)}
          placeholder="~1990"
          isEditing={isEditing}
        />
        ...
        <div className="col-span-2">
          <EditField
            label="Parking Notes"
            icon={Car}
            value={formData.property_details.parking}
            onChange={(v: string) =>
              updateNested("property_details", "parking", v)
            }
            type="textarea"
            placeholder="Driveway ok, or Street Park only"
            isEditing={isEditing}
          />
        </div>
      </div>
...
// ------------------------------------------------------------------
export const HouseholdVibeCard = ({
  formData,
  updateNested,
  isEditing,
  mode = "admin",
  ...
  return (
    <GlassCard className="p-6 space-y-4">
      <h2 className="text-lg font-bold flex items-center gap-2 text-orange-300 border-b border-white/10 pb-2">
        ...
        {mode === "client" ? "Household Info" : "Household Vibe"}
      </h2>
      <div className="grid grid-cols-1 gap-4">
        <EditField
          label="Spouse / Partner"
          icon={User}
          value={formData.personal_details.spouse}
          onChange={(v: string) =>
            updateNested("personal_details", "spouse", v)
          }
          isEditing={isEditing}
        />
        <EditField
          label="Kids"
          icon={User}
          value={formData.personal_details.kids}
          onChange={(v: string) => updateNested("personal_details", "kids", v)}
        ...
        <div className="group bg-orange-500/5 p-3 rounded-lg border border-orange-500/10">
          <EditField
            label="Pets (Crucial!)"
            icon={Dog}
            value={formData.personal_details.pets}
            onChange={(v: string) =>
              updateNested("personal_details", "pets", v)
            }
            placeholder="Name (Type) - Temperament"
            isEditing={isEditing}
          />
        </div>
        ...
        <EditField
          ...
          onChange={(v: string) =>
            updateNested("personal_details", "hobbies", v)
          }
          isEditing={isEditing}
        />
        ...
        <div className="flex gap-2 pt-2">
          <div
            className={`px-2 py-1 rounded text-xs border cursor-pointer ${formData.personal_details.is_veteran ? "bg-indigo-500/20 text-indigo-300 border-indigo-500/30" : "bg-transparent border-white/10 text-zinc-500"}`}
            onClick={() =>
              isEditing &&
              updateNested(
                "personal_details",
                "is_veteran",
                !formData.personal_details.is_veteran,
              )
            }
          >
...
// ------------------------------------------------------------------
export const OperationalIntelCard = ({
  formData,
  updateNested,
  isEditing,
  mode = "admin",
  ...
  return (
    <GlassCard className="p-6 space-y-4">
      <h2 className="text-lg font-bold flex items-center gap-2 text-red-300 border-b border-white/10 pb-2">
        ...
        {mode === "client" ? "Service Preferences" : "Operational Intel"}
      </h2>
      <div className="space-y-4">
        <div className="bg-red-500/10 p-3 rounded-lg border border-red-500/20">
          <EditField
            ...
            onChange={(v: string) =>
              updateNested("preferences", "allergies", v)
            }
            placeholder="Dust, noise, chemicals"
            isEditing={isEditing}
          />
        </div>
        <EditField
          ...
          icon={Clock}
          value={formData.preferences.work_hours}
          onChange={(v: string) => updateNested("preferences", "work_hours", v)}
          placeholder="No start before 9am"
          isEditing={isEditing}
        />
        <EditField
          label="Neighbor Considerations"
          icon={Users}
          value={formData.preferences.neighbors}
          onChange={(v: string) => updateNested("preferences", "neighbors", v)}
          placeholder="Warn neighbor at #42"
          isEditing={isEditing}
        />
        <EditField
          label="Property Quirks"
          icon={Layers}
          value={formData.property_details.quirks}
          onChange={(v: string) =>
            updateNested("property_details", "quirks", v)
          }
          type="textarea"
          placeholder="Breaker in garage behind shelf"
          isEditing={isEditing}
        />
      </div>
    </GlassCard>
...
// ------------------------------------------------------------------
export const FutureOpportunitiesCard = ({
  formData,
  updateNested,
  isEditing,
  mode = "admin",
  ...
  const addOpportunity = () => {
    ...
    updateNested("property_details", "future_opportunities", [
      ...currentOps,
  ...
  const updateOpportunity = (id: number, text: string) => {
    const currentOps = formData.property_details.future_opportunities || [];
    const updatedOps = currentOps.map((op: any) =>
      op.id === id ? { ...op, text } : op,
    );
      ...
      <div className="flex justify-between items-center border-b border-white/10 pb-2">
        ...
        {isEditing && (
          <button
            ...
            className="bg-purple-500/20 hover:bg-purple-500/40 text-purple-300 p-1 rounded transition-colors"
          >
