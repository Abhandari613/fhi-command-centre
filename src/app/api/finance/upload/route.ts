  ...
  try {
    ...
    if (!profile?.organization_id) {
      return NextResponse.json(
        { error: "No organization found based on user profile" },
        { status: 403 },
      );
    }
    ...
    const descKey = findKey(firstRow, [
      "desc",
      "memo",
      "narrative",
      "details",
    ...
    if (!dateKey || (!amountKey && !debitKey && !creditKey)) {
      return NextResponse.json(
        {
          error:
            "Could not detect required columns (Date, Amount/Debit/Credit). Please ensure headers are present.",
        },
        { status: 400 },
      );
    }
