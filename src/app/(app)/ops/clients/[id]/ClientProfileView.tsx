...
import {
  EditField,
  AddressEditField,
} from "@/components/clients/ClientProfileComponents";
import {
  PropertyIntelCard,
  HouseholdVibeCard,
...
export function ClientProfileView({
  ...
  const activeJobs =
    jobs?.filter((j) => j.status === "in_progress" || j.status === "scheduled")
      .length || 0;
  const totalRevenue =
    jobs?.reduce((acc, job) => acc + (job.total_amount || 0), 0) || 0;
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
          ...
          <div className="flex-1 w-full space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-white/5">
              ...
              <EditField
                ...
                onChange={(v: string) =>
                  updateNested("preferences", "preferred_channel", v)
                }
                placeholder="Select..."
                isEditing={isEditing}
              />
        ...
        <div className="space-y-6">
          ...
          <GlassCard className="p-6">
            ...
            <EditField
              ...
              onChange={(val) =>
                updateNested("preferences", "preferred_channel", val)
              }
              isEditing={isEditing}
              type="select"
              options={[
              ...
              <GlassCard className="p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <EditField
                    label="Lead Source"
                    icon={Tag}
                    value={formData.marketing_info.source}
                    onChange={(v: string) =>
                      updateNested("marketing_info", "source", v)
                    }
                  ...
                  <EditField
                    label="Referred By"
                    icon={User}
                    value={formData.marketing_info.referred_by}
                    onChange={(v: string) =>
                      updateNested("marketing_info", "referred_by", v)
                    }
                    isEditing={isEditing}
                  />
                  <EditField
                    label="Client Status"
                    icon={Layers}
                    value={formData.marketing_info.status}
                    onChange={(v: string) =>
                      updateNested("marketing_info", "status", v)
                    }
                    placeholder="Active, Lead, VIP"
                    isEditing={isEditing}
                  />
      ...
      {isEditing && (
        <div className="fixed bottom-6 right-6 z-50">
          <AnimatedButton
            onClick={handleSave}
            className="bg-green-500 text-black font-bold shadow-2xl shadow-green-500/20 hover:scale-105"
          >
