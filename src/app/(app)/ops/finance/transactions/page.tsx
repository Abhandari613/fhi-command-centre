...
export default function TransactionsPage() {
    ...
    const handleUpdate = async (id: string, updates: TransactionUpdate) => {
        ...
        setTransactions(prev => prev.map(t => t.id === id ? { ...t, ...updates } as unknown as Transaction : t));
    ...
    return (
        <div className="flex flex-col gap-6 pb-24">
            <header className="flex items-center gap-4">
                <Link href="/ops/finance" className="p-2 -ml-2 rounded-full hover:bg-white/10 transition-colors">
                    ...
                    <GlassCard key={tx.id} className="p-4 flex flex-col gap-3 group">
                        <div className="flex justify-between items-start">
                            <div className="flex gap-3">
                                <button
                                    onClick={() => handleUpdate(tx.id, { review_status: tx.review_status === 'reviewed' ? 'unreviewed' : 'reviewed' })}
                                    className={cn(
                                        "w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold transition-all",
                                        tx.review_status === 'reviewed' ? "bg-emerald-500/20 text-emerald-400" : "bg-white/5 text-white/40 hover:bg-white/10"
                                    )}>
                                    {tx.review_status === 'reviewed' ? <Check className="w-5 h-5" /> : "?"}
                                </button>
                                <div>
                        ...
                        <div className="flex flex-col gap-2">
                            <div className="flex gap-2">
                                ...
                                <select
                                    className={cn(
                                        ...
                                        (tx as any).work_order_id ? "bg-blue-500 border-blue-500 text-blue-300" : "bg-white/5 border-white/10 text-white/60 text-center border-dashed"
                                    )}
                                    value={(tx as any).work_order_id || ""}
                                    onChange={(e) => handleUpdate(tx.id, { work_order_id: e.target.value || null } as any)}
                                >
                                    <option value="">+ Assign Work Order</option>
                                    {workOrders.map(wo => (
                                        <option key={wo.id} value={wo.id}>{wo.property_address_or_unit}</option>
                                    ))}
                                </select>
                                ...
                                <select
                                    className={cn(
                                        "flex-1 appearance-none bg-opacity-20 border border-opacity-30 rounded-lg px-3 py-2 text-xs font-bold focus:outline-none transition-colors",
                                        tx.category_id ? "bg-amber-500 border-amber-500 text-amber-300" : "bg-white/5 border-white/10 text-white/60 text-center border-dashed"
                                    )}
                                    value={tx.category_id || ""}
                                    onChange={(e) => handleUpdate(tx.id, { category_id: e.target.value || null })}
                                >
                            ...
                            <div className="relative">
                                <FileText className={cn("absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 pointer-events-none", tx.receipt_id ? "text-purple-300" : "opacity-30")} />
                                <select
                                    className={cn(
                                        ...
                                        tx.receipt_id ? "bg-purple-500 border-purple-500 text-purple-300" : "bg-white/5 border-white/10 text-white/60 border-dashed"
                                    )}
                                    value={tx.receipt_id || ""}
                                    onChange={(e) => handleUpdate(tx.id, { receipt_id: e.target.value || null })}
                                >
                                    <option value="">Attach Receipt...</option>
                                    {receipts.map(r => (
                                        <option key={r.id} value={r.id}>
                                            ${r.total} - {r.merchant} ({new Date(r.date).toLocaleDateString()})
                                        </option>
                                    ))}
                                </select>
                            </div>
