...
export default function QuoteDetailsPage() {
  ...
  const calculateTotal = () => {
    return (
      quote?.quote_line_items?.reduce(
        (sum: number, item: QuoteLineItemRow) => sum + (item.total || 0),
        0,
      ) || 0
    );
  };
  ...
  const handleConvertToJob = async () => {
    if (
      !confirm("Are you sure you want to convert this quote to an active job?")
    )
      return;
    setConverting(true);
  ...
  return (
    <div className="relative min-h-screen pb-24 overflow-hidden">
      ...
      <div className="p-6 flex flex-col gap-6 relative z-10">
        ...
        >
          <GlassCard intensity="panel" className="p-6 space-y-8">
            ...
            >
              <GlassCard
                intensity="normal"
                className="p-6 bg-white/5 border-none"
              >
                <JobPhotoGallery
                  jobId={quote.id}
                  photos={photos}
            ...
            {/* Job Description */}
            <div>
              <h3 className="text-[10px] opacity-40 uppercase font-bold tracking-widest mb-3">
                Scope of Work
              </h3>
              <GlassCard
                intensity="normal"
                className="p-4 bg-white/5 border-none"
              >
        ...
        {["in_progress", "completed"].includes(quote.status || "") && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        >
          <GlassCard className="p-6">
            ...
            {assignedSubs.length === 0 ? (
              <div className="text-sm opacity-50 italic text-center py-4">
        ...
        {/* Payments Section (In Progress+ Jobs) */}
        {["in_progress", "completed", "invoiced", "paid"].includes(
          quote.status || "",
        ) && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="bg-white/5 p-4 rounded-2xl text-center border border-white/5">
                  ...
                  <div className="font-bold font-mono text-sm text-amber-400">
                    ${(calculateTotal() - calculatePaid()).toFixed(2)}
                  </div>
                </div>
              </div>
