      ...
      if (txn.amount > 0) {
        ...
        const matchedJob = (jobs || []).find(
          (j: any) =>
            ...
            Math.abs(Number(j.final_invoice_amount) - Number(txn.amount)) <
              0.01,
        );
        if (matchedJob) {
...
export async function matchTransactionToWorkOrder(
  txnId: string,
  workOrderId: string,
) {
  const supabase = await createClient();
...
export async function createCategorizationRule(
  ...
  const { error } = await supabase.from("finance_rules").insert({
    ...
    match_type: "CONTAINS",
    organization_id: (await supabase.auth.getUser()).data.user?.user_metadata
      .organization_id, // simplistic - should be safer
  });
