  ...
  return withActionValidation(
    ...
    async (validatedData) => {
      ...
      if (authError || !user) {
        return {
          success: false,
          error: createActionError("UNAUTHORIZED", "Unauthorized access", 401),
        };
      }
      ...
      const computedExpectedSupplies = [...(expected_supplies || [])];
      ...
      const { data, error: jobError } = await supabase
        ...
        .insert({
          ...
          estimated_duration,
          requires_supplies:
            requires_supplies || computedExpectedSupplies.length > 0,
      ...
      if (line_items.length > 0 && job?.id) {
        ...
        if (itemsError) {
          ...
          return {
            ...
            error: createActionError(
              "DB_INSERT_FAILED",
              "Quote created but items failed to save",
              500,
            ),
          };
        }
