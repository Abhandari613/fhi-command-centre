...
export async function dispatchJobToSub(
  jobId: string,
  subcontractorId: string,
): Promise<ActionResult<{ assignment: Assignment; magicLink: string }>> {
  const supabase =
  ...
  return withActionValidation(createSubSchema, input, async (validatedData) => {
    ...
    if (!user) {
      return {
        ...
        error: createActionError(
          "UNAUTHORIZED",
          "Unauthorized payload execution",
          401,
        ),
      };
    }
    ...
    if (!profile?.organization_id) {
      return {
        ...
        error: createActionError(
          "MISSING_ORG",
          "No organization found for the authenticated user",
          403,
        ),
      };
    }
    ...
    if (error) {
      ...
      return {
        ...
        error: createActionError(
          "DB_INSERT_FAILED",
          error.message || "Failed to create subcontractor",
          500,
        ),
      };
    }
